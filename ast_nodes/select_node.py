...
class SelectNode(ASTNode):
    ...
    def _get_random_column_reference(self) -> Optional[ColumnReferenceNode]:
        """Get a random column reference，Prefer to use column_tracker to select unused columns"""
        if not self.from_clause or not hasattr(self.from_clause, 'get_table_alias_map'):
            print(f"[SelectNode] Could not get table alias mapping，BackNone")
            return None

        table_alias_map = self.from_clause.get_table_alias_map()
        if not table_alias_map:
            print(f"[SelectNode] Table alias mapping is empty，BackNone")
            return None

        table_name = random.choice(list(table_alias_map.keys()))
        table = next((t for t in self.tables if t.name == table_name), None)
        if not table:
            print(f"[SelectNode] No DB tables found.: {table_name}，BackNone")
            return None

        table_alias = table_alias_map[table_name]
        
        #If there is a column_tracker, use it to select unused columns
        if self.column_tracker:
            print(f"[SelectNode] Use column tracker to select tables {table_alias} Columns of")
            column = get_random_column_with_tracker(table, table_alias, self.column_tracker, for_select=True)
            if column:
                print(f"[SelectNode] Successfully selected columns via column tracker: {table_alias}.{column.name}")
                return ColumnReferenceNode(column, table_alias)
            print(f"[SelectNode] Column tracker failed to select a valid column，Fallback logic will be used")
        else:
            print(f"[SelectNode] No column tracker set，Use original logic")
        
        #Fallback to original logic if no column_tracker or no columns available
