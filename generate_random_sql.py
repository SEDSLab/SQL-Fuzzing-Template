"""Compatibility facade for SQL generation utilities.

This module re-exports functions from `sql_generation.random_sql.*` so older
imports (for example `from generate_random_sql import ...`) continue to work.
...
def get_tables():
    """Return the current in-memory table registry used by the generator."""
    return _state.get_tables()


def set_tables(tables_list):
    """Replace the in-memory table registry used by the generator."""
    _state.set_tables(tables_list)


def _get_subquery_depth() -> int:
    """Return current global subquery depth state."""
    return _state.get_subquery_depth()


def _set_subquery_depth(depth: int) -> None:
    """Update global subquery depth state."""
