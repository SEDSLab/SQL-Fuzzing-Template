    ...
    if has_aggregate_function:
        ...
        if random.random() > 0.5:
            ...
            if agg_funcs:
                ...
                #Select the appropriate column for the aggregate function
                #Get column tracker
                column_tracker = select_node.metadata.get('column_tracker')
                ...
                #Select the appropriate column based on the query type
                if use_subquery and hasattr(from_node.table_references[0], 'column_alias_map'):
                    subquery_node = from_node.table_references[0]
                    #Select from the output columns of the subquery
                    valid_aliases = list(subquery_node.column_alias_map.keys())
                    if valid_aliases:
                        candidate_aliases = valid_aliases
                        if numeric_required:
                    ...
                    #Initialize the valid_columns variable by default, making sure it always has a value
                    ...
                    if not param_added and column_tracker:
                        ...
                        #Ensure there is at least one valid column
